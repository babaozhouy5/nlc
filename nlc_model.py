...
class GRUCellAttn(tf.contrib.rnn.GRUCell):
    ...
    def __init__(self, num_units, encoder_output, scope=None):
        ...
        with tf.variable_scope(scope or type(self).__name__):
            with tf.variable_scope("Attn1"):
                ...
                phi_hs2d = tf.tanh(core_rnn_cell_impl._linear(
                    hs2d, num_units, True, 1.0))
                self.phi_hs = tf.reshape(phi_hs2d, tf.shape(self.hs))
        super(GRUCellAttn, self).__init__(num_units)

    def __call__(self, inputs, state, scope=None):
        gru_out, gru_state = super(
            GRUCellAttn, self).__call__(inputs, state, scope)
        with tf.variable_scope(scope or type(self).__name__):
            with tf.variable_scope("Attn2"):
                gamma_h = tf.tanh(core_rnn_cell_impl._linear(
                    gru_out, self._num_units, True, 1.0))
            weights = tf.reduce_sum(
                self.phi_hs * gamma_h, reduction_indices=2, keep_dims=True)
            weights = tf.exp(weights - tf.reduce_max(weights,
                                                     reduction_indices=0, keep_dims=True))
            weights = weights / \
                (1e-6 + tf.reduce_sum(weights, reduction_indices=0, keep_dims=True))
            context = tf.reduce_sum(self.hs * weights, reduction_indices=0)
            with tf.variable_scope("AttnConcat"):
                out = tf.nn.relu(core_rnn_cell_impl._linear(
                    [context, gru_out], self._num_units, True, 1.0))
...
class NLCModel(object):
    ...
    def __init__(self, vocab_size, size, num_layers, max_gradient_norm, batch_size, learning_rate,
        ...
        self.target_length = tf.reduce_sum(
            self.target_mask, reduction_indices=0)
...
#      self.gradient_norm = tf.global_norm(clipped_gradients)
    ...
    def setup_embeddings(self):
        with tf.variable_scope("embeddings"):
            ...
            self.encoder_inputs = tf.nn.embedding_lookup(
                self.L_enc, self.source_tokens)
            self.decoder_inputs = tf.nn.embedding_lookup(
                self.L_dec, self.target_tokens)

    def setup_encoder(self):
        #self.encoder_cell = tf.contrib.rnn.GRUCell(self.size)
        with tf.variable_scope("PryamidEncoder"):
            inp = self.encoder_inputs
            mask = self.source_mask
            out = None
            for i in xrange(self.num_layers):
                with tf.variable_scope("EncoderCell%d" % i) as scope:
                    srclen = tf.reduce_sum(mask, reduction_indices=0)
                    out, _ = self.bidirectional_rnn(inp, srclen, scope=scope)
    ...
    def setup_decoder(self):
        ...
        with tf.variable_scope("Decoder"):
            ...
            for i in xrange(self.num_layers - 1):
                with tf.variable_scope("DecoderCell%d" % i) as scope:
                    decoder_cell = tf.contrib.rnn.GRUCell(self.size)
                    self.decoder_cells.append(decoder_cell)
                    out, state_output = tf.nn.dynamic_rnn(decoder_cell, inp, time_major=True,
                                                          dtype=tf.float32, sequence_length=self.target_length,
                                                          scope=scope, initial_state=self.decoder_state_input[i])
                    inp = self.dropout(out)
                    self.decoder_state_output.append(state_output)

            with tf.variable_scope("DecoderAttnCell") as scope:
                out, state_output = tf.nn.dynamic_rnn(self.attn_cell, inp, time_major=True,
                                                      dtype=tf.float32, sequence_length=self.target_length,
    ...
    def decoder_graph(self, decoder_inputs, decoder_state_input):
        ...
        with tf.variable_scope("Decoder", reuse=True):
            for i in xrange(self.num_layers - 1):
                with tf.variable_scope("DecoderCell%d" % i) as scope:
                    inp, state_output = self.decoder_cells[i](
                        inp, decoder_state_input[i])
                    decoder_state_output.append(state_output)

            with tf.variable_scope("DecoderAttnCell") as scope:
                decoder_output, state_output = self.attn_cell(
                    inp, decoder_state_input[i + 1])
                decoder_state_output.append(state_output)
    ...
    def setup_beam(self):
        ...
        def beam_step(time, beam_probs, beam_seqs, cand_probs, cand_seqs, *states):
            ...
            inputs = tf.reshape(tf.slice(beam_seqs, [0, time], [
        ...
        var_shape.extend([(state_0, tf.TensorShape([None, self.size]))
                          for state_0 in states_0])
        loop_vars, loop_var_shapes = zip(* var_shape)
        ret_vars = tf.while_loop(cond=beam_cond, body=beam_step, loop_vars=loop_vars,
                                 shape_invariants=loop_var_shapes, back_prop=False)
#    time, beam_probs, beam_seqs, cand_probs, cand_seqs, _ = ret_vars
    ...
    def setup_loss(self):
        with tf.variable_scope("Logistic"):
            ...
            self.outputs = tf.reshape(outputs2d, tf.stack(
                [T, batch_size, self.vocab_size]))
            ...
            labels1d = tf.reshape(
                tf.pad(targets_no_GO, [[0, 1], [0, 0]]), [-1])
    ...
    def downscale(self, inp, mask):
        ...
        with vs.variable_scope("Downscale"):
            ...
            inp2d = tf.reshape(tf.transpose(
                inp, perm=[1, 0, 2]), [-1, 2 * self.size])
            out2d = core_rnn_cell_impl._linear(inp2d, self.size, True, 1.0)
            out3d = tf.reshape(out2d, tf.stack(
                (batch_size, tf.to_int32(T / 2), dim)))
    ...
    def bidirectional_rnn(self, inputs, lengths, scope=None):
        ...
        with tf.variable_scope(name + "_BW") as bw_scope:
            output_bw, output_state_bw = tf.nn.dynamic_rnn(cell_bw, inputs_bw, time_major=True, dtype=tf.float32,
                                                           sequence_length=lengths, scope=bw_scope)

        output_bw = tf.reverse_sequence(
            output_bw, tf.to_int64(lengths), seq_dim=0, batch_dim=1)
    ...
    def train(self, session, source_tokens, source_mask, target_tokens, target_mask):
        ...
        self.set_default_decoder_state_input(
            input_feed, target_tokens.shape[1])
    ...
    def decode(self, session, encoder_output, target_tokens, target_mask=None, decoder_states=None):
        ...
        input_feed[self.target_tokens] = target_tokens
        input_feed[self.target_mask] = target_mask if target_mask else np.ones_like(
            target_tokens)
        input_feed[self.keep_prob] = 1.
